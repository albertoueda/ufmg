...
void init_dp_table()
{
    dp_table.resize(MAX_MSG_SIZE + 2);
...
{
    // cout << "calculating for: " << i << " " << counter_1 << " " << counter_0 << endl;
...
int main(int argc, char** argv)
{
    ...
    if (argc == 1)
    {
        filename = "all_input_n_10.txt"; // string_100_michel experiment_paulo_input  input  all_input_n_7  strings-tp-description
    }
    else if (argc > 1)
    {
        filename = argv[1];
    ...
    for (int i = 0; i < number_of_instances; ++i)
    {
        ...
        */
        ...
        /*
        duration = (clock() - start) * 1000000.0 / (double) CLOCKS_PER_SEC;
        // cout << duration << endl;
        // cout << i+1 << ":" << duration << endl;
        // cout << " in " << duration << " microseg" << endl;
        */

        if (answer_dp != answer_ga)
        {
            if (answer_dp == BOTH) continue;
